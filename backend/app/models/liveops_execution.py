...
Together "liveops_executions" table. One row per event_id: the UNIQUE
constraint on event_id is the database-level exactly-once guard. A repeated
execute request for an already-executed event hits the constraint (or the
status check) and returns HTTP 409 without touching the simulated cloud.
...
This model deliberately lives OUTSIDE decisions/events: the audit trail for
executions is separate from evaluation results, and neither the DecisionORM nor
the EventORM schemas are touched.
"""
...
class LiveOpsExecutionResponse(BaseModel):
    ...
    result is exposed as a parsed dict (or null); no filesystem paths are
    ever included in the stored result payload.
    """
