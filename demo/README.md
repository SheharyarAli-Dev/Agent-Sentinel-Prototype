...
2. Double-click **`setup_demo.bat`**.
   This:
   - creates `backend\.venv` only if it does not exist (never deletes it)
   - upgrades pip inside the venv
   - installs `backend/requirements.txt`
   - runs `npm.cmd ci` in `frontend`
   - downloads and verifies `sentence-transformers/all-MiniLM-L6-v2`
     (first run downloads ~90 MB and can take a few minutes)
...
- enables **offline model mode** (`HF_HUB_OFFLINE=1 TRANSFORMERS_OFFLINE=1`)
  when the MiniLM cache is already present
...
> before it reports healthy. Wait for the dashboard browser window to open
> automatically; do not close the backend or frontend windows while it is
> starting. Subsequent starts are faster because the model is cached.
...
- whether MiniLM loads from the local cache
- backend test result
- frontend build result
- whether ports 8000 and 5173 are available or occupied
...
| `npm not found` | Install Node.js and add it to `PATH`, then re-run `setup_demo.bat`. |
| `python not found` | Install Python 3.10+ and add it to `PATH`, then re-run `setup_demo.bat`. |
| dirty Git working tree | Uncommitted changes exist; see "How to update safely" below. |
